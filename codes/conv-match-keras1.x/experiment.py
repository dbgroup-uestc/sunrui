...
from babi_helper import get_stories, vectorize_stories, tar
...
BATCH_SIZE = 64*2 # BATCH_SIZE
EPOCHS = 50   # 训练轮数

ONLY_SUPPORTING = False

np.set_printoptions(precision=4) #设置控制台打印的精度
np.set_printoptions(suppress=True) # 控制台打印时不用科学计数法

# optparse都是命令行相关，这里可以先不考虑
